...
def test_runtime_loader_plans_executable_msty_reference_with_explicit_endpoint(tmp_path: Path) -> None:
	server = OllamaFixtureServer(
		models={
			"llama3.2": {
...
def test_runtime_loader_marks_msty_reference_non_executable_without_endpoint(tmp_path: Path) -> None:
...
def test_runtime_loader_discovers_msty_models_with_explicit_endpoint(tmp_path: Path) -> None:
	server = OllamaFixtureServer(
		models={
			"llama3.2": {"capabilities": ["completion"], "response_text": "ready"},
		}
