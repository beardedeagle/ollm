...
def test_openai_responses_accept_input_file_parts(monkeypatch) -> None:
    configure_response_store(monkeypatch, backend="memory")
    application_service = build_application_service()
    app = create_server_app(application_service)
    client = build_test_client(app)
...
def test_openai_responses_stream_response_events(monkeypatch) -> None:
    ...
    done_payload = payloads[5]
    content_done_payload = payloads[6]
    output_item_done_payload = payloads[7]
    completed_payload = payloads[8]
    ...
    assert content_done_payload["type"] == "response.content_part.done"
...
def test_openai_responses_delete_removes_stored_response(monkeypatch) -> None:
    configure_response_store(monkeypatch, backend="memory")
    app = create_server_app(build_application_service())
    client = build_test_client(app)
    ...
    response_id = cast(str, json_object(create_response)["id"])

    delete_response = cast(
        JsonResponseProtocol,
        client.delete(f"/v1/responses/{response_id}"),
    )
    delete_payload = json_object(delete_response)
    fetch_response = cast(
        JsonResponseProtocol,
        client.get(f"/v1/responses/{response_id}"),
    )
    fetch_payload = json_object(fetch_response)
    fetch_error = payload_dict(fetch_payload["error"])
    ...
    assert fetch_error["type"] == "not_found_error"


def test_openai_responses_delete_is_disabled_without_store() -> None:
    app = create_server_app(build_application_service())
    client = build_test_client(app)
