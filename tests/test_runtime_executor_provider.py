...
def test_runtime_executor_streams_ollama_provider_output(tmp_path: Path) -> None:
    ...
    assert sink.status_messages == [
        "Running ollama:llama3.2 via provider backend ollama"
    ]
...
def test_runtime_executor_sends_base64_image_to_ollama_vision_model(
    tmp_path: Path,
) -> None:
    server = OllamaFixtureServer(
        models={
            ...
            messages=[
                Message(
                    ...
                    content=[
                        ...
                        ContentPart.image(str(image_path)),
                    ],
                )
            ],
        )
    ...
    assert images == [base64.b64encode(b"png-bytes").decode("ascii")]


def test_runtime_executor_fetches_remote_image_url_for_ollama_vision_model(
    tmp_path: Path,
) -> None:
    server = OllamaFixtureServer(
        models={
    ...
    images = cast(dict[str, object], first_message)["images"]
    assert isinstance(images, list)
    assert images == [base64.b64encode(b"remote-png-bytes").decode("ascii")]


def test_runtime_executor_rejects_non_image_remote_url_for_ollama_vision_model(
    tmp_path: Path,
) -> None:
    server = OllamaFixtureServer(
        models={
