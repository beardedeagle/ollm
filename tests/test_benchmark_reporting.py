...
def test_render_runtime_probe_json_round_trips() -> None:
    ...
    assert cache_state["window_max_tokens"] is None
    assert cache_state["eviction_policy"] is None
    ...
    assert cache_state["eviction_count"] == 0
    assert cache_state["evicted_tokens"] == 0
...
def test_summarize_request_metrics_includes_native_runtime_profile() -> None:
    ...
    eviction_count = cast(dict[str, object], cache_state["eviction_count"])
    assert eviction_count["mean"] == 0.0
