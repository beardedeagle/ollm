        ...
        class OpenAICompatibleHandler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:  # noqa: N802
                fixture.requests.append(
                    RecordedRequest(method="GET", path=self.path, payload=None)
                )
                if self.path == "/v1/models":
                    ...
                    return
                _write_json(
                    self, 404, {"error": {"message": f"unknown path: {self.path}"}}
                )

            def do_POST(self) -> None:  # noqa: N802
                payload = _read_json_body(self)
                fixture.requests.append(
                    RecordedRequest(method="POST", path=self.path, payload=payload)
                )
...
def _handle_chat(
    ...
    if model is None:
        _write_json(
            handler, 404, {"error": {"message": f"model '{model_name}' not found"}}
        )
        return
...
def _write_json(
    handler: BaseHTTPRequestHandler, status_code: int, payload: dict[str, object]
) -> None:
