...
class MyGptOssExperts(GptOssExperts):
    def forward(
    ...
    ) -> torch.Tensor:  # chunked/looped
        num_experts = routing_weights.shape[1]
        batch_size, T, H = hidden_states.shape
        acc = torch.zeros(
            (batch_size, T, H), device=hidden_states.device, dtype=hidden_states.dtype
        )
        for e in range(num_experts):
            ...
            gate_up = (
                torch.bmm(hidden_states, s_gate_up_proj)
                + s_gate_up_proj_bias[..., None, :]
            )
            ...
            next_states = (
                next_states
                * routing_weights_e.transpose(0, 1).view(1, batch_size, -1)[..., None]
            )
...
class MyGptOssModel(GptOssModel):
    def __init__(self, config: GptOssConfig):
        ...
        self.layers = nn.ModuleList(
            [MyGptOssDecoderLayer(config, layer_idx) for layer_idx in range(2)]
        )
...
def my_eager_attention_forward(
    ...
    **kwargs,
    ...
    # Flash-attention
    offset = min(
        key.shape[2] - query.shape[2], sliding_window if sliding_window else 999
    )
