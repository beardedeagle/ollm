...
def _match_group(primary: re.Match[str] | None, secondary: re.Match[str] | None) -> str | None:
...
class GDSWeights:
	...
	def load_from_disk_to_cuda(self, path: str, shape: list[int], dtype: str) -> torch.Tensor:
		cupy = _require_cupy()
	...
	def load_mxfp4_from_disk(self, path: str) -> torch.Tensor:
		packed_tensors = cast(dict[str, torch.Tensor], torch.load(path, map_location=self.device))
		return convert_moe_packed_tensors(packed_tensors["_blocks"], packed_tensors["_scales"]).to(self.device)
	...
	def offload_param_to_cpu(self, name: str) -> None:
		...
		if packed == "mxfp4" or dtype.startswith("torch"):
			tensor = cast(torch.Tensor | dict[str, torch.Tensor], torch.load(path, map_location="cpu"))
		else:
			tensor = self.load_from_disk_to_cuda(path, shape, dtype).cpu()
		self.offloaded_map[name] = _OffloadedTensorRecord(shape=shape, dtype=dtype, packed=packed, tensor=tensor)
...
def convert_moe_packed_tensors(
	...
	*prefix_shape, group_count, block_size = blocks.shape
	...
	output = output.reshape(*prefix_shape, group_count, block_size * 2).view(*prefix_shape, group_count * block_size * 2)
...
class SafeTensorReader:
	def __init__(self, path: str):
		...
		with open(path, "rb") as handle:
			...
			self.header = cast(dict[str, _SafeTensorHeaderEntry], json.loads(handle.read(header_length)))
...
class SafeTensorReaderGPU:
	...
	def get_tensor(self, name: str) -> torch.Tensor:
		...
		future = self._file_pointer.pread(buffer, file_offset=self.data_offset + offset_start)
...
def get_optimal_safetensor_reader(filepath: str, device: torch.device | str | None = None) -> ReaderType:
	if kvikio_available:
...
class DenseWeightsLoader:
	def __init__(self, path: str, device: str = "cuda:0"):
		...
		for manifest_name, filename in weight_map.items():
			...
			if base is None:
				continue
			self.manifest.setdefault(base, {})[manifest_name.replace(base, "")] = filename
		self.device = torch.device(device)
	...
	def get_offloaded_dict_to_cuda(self, base: str) -> dict[str, torch.Tensor] | None:
		if base not in self.offloaded_map:
			return None
		return {attr_path: tensor.to(self.device, non_blocking=True) for attr_path, tensor in self.offloaded_map[base].items()}

	def load_dict_from_disk(self, base: str, device: torch.device | str = "cpu") -> dict[str, torch.Tensor]:
		return {
			attr_path: self.safetensors[filename].get_tensor(base + attr_path).to(device)
