...
class OllamaBackend(ExecutionBackend):
    ...
    def __init__(
        ...
        self._client_factory = (
            OllamaClient if client_factory is None else client_factory
        )
    ...
    def refine_plan(self, plan: RuntimePlan, config: RuntimeConfig) -> RuntimePlan:
            ...
            return self._provider_failure_plan(
                ...
                reason,
                {
                    "provider_endpoint": _provider_endpoint_details(
                        provider_name, config, self._default_client.base_url
                    )
                },
            )
        try:
        ...
        return self._provider_success_plan(
            plan, provider_name, client.base_url, model_details
        )
    ...
    def load(self, plan: RuntimePlan, config: RuntimeConfig) -> BackendRuntime:
        ...
        if provider_name not in {"ollama", "msty"}:
            raise ValueError(
                "ollama backend requires an ollama: or msty: model reference"
            )
        client = self._client_from_config(provider_name, config)
    ...
    def _provider_success_plan(
        ...
        reason = f"{provider_name} model '{model_details.name}' is executable via {provider_endpoint}."
        capabilities = provider_capabilities(
            ...
            details={
                ...
                "parameter_size": (
                    ""
                    if model_details.parameter_size is None
                    else model_details.parameter_size
                ),
                "quantization_level": (
                    ""
    ...
    def _client_from_config(
        self, provider_name: str, config: RuntimeConfig
    ) -> OllamaClient:
