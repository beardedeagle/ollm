...
from ollm.runtime.backends.base import (
    BackendRuntime,
...
class OpenAICompatibleBackend(ExecutionBackend):
    ...
    def __init__(
        ...
        self._client_factory = (
            OpenAICompatibleClient if client_factory is None else client_factory
        )
    ...
    def refine_plan(self, plan: RuntimePlan, config: RuntimeConfig) -> RuntimePlan:
        ...
        if config.resolved_adapter_dir() is not None:
            ...
            return self._provider_failure_plan(
                plan, reason, config.resolved_provider_endpoint()
            )
        try:
            provider_endpoint = _resolve_provider_endpoint(provider_name, config)
        except ValueError as exc:
            return self._provider_failure_plan(
                plan, str(exc), config.resolved_provider_endpoint()
            )
    ...
    def load(self, plan: RuntimePlan, config: RuntimeConfig) -> BackendRuntime:
        ...
        if provider_name not in {"openai-compatible", "lmstudio"}:
            raise ValueError(
                "openai-compatible backend requires an openai-compatible: or lmstudio: model reference"
            )
        provider_endpoint = _resolve_provider_endpoint(provider_name, config)
        client = self._client_factory(provider_endpoint)
        return BackendRuntime(
            ...
            execute_prompt=lambda request, sink: self._execute_prompt(
                client, plan, request, sink
            ),
...
def _resolve_discovery_endpoint(
    provider_name: str, provider_endpoint: str | None
) -> str:
    if provider_endpoint is not None:
...
def _validate_provider_offload(config: RuntimeConfig) -> None:
    if config.offload_cpu_layers > 0 or config.offload_gpu_layers > 0:
        raise ValueError(
            "Provider-backed backends do not support custom layer offload controls"
        )
...
def _openai_message(message: Message) -> dict[str, object]:
    ...
    return {
        ...
        "content": [
            _openai_content_part(part.kind, part.value) for part in message.content
        ],
    }
...
def _openai_content_part(kind: ContentKind, value: str) -> dict[str, object]:
    ...
    if kind is ContentKind.IMAGE:
        raise ValueError(
            "The openai-compatible backend currently supports text and audio inputs only"
        )
