...
class OllamaClient:
    def __init__(
        self, base_url: str = DEFAULT_OLLAMA_ENDPOINT, timeout_seconds: float = 300.0
    ):
    ...
    def list_models(self) -> tuple[str, ...]:
        ...
        if not isinstance(raw_models, list):
            raise OllamaClientError(
                "Expected 'models' list from Ollama /api/tags response"
            )
    ...
    def show_model(self, model_name: str) -> OllamaModelDetails:
        ...
        raw_capabilities = tuple(
            str(item) for item in _payload_list(payload, "capabilities")
        )
    ...
    def _request_json(
        self, path: str, payload: dict[str, object] | None
    ) -> dict[str, object]:
        request = Request(
