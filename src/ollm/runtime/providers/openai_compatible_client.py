...
class OpenAICompatibleClient:
	...
	def list_models(self) -> tuple[str, ...]:
		...
		if not isinstance(raw_models, list):
			raise OpenAICompatibleClientError("Expected 'data' list from provider /models response")
		model_ids: list[str] = []
	...
	def chat_completions(
		...
		if not stream:
			...
			return OpenAICompatibleChatResult(
				...
				metadata=_chat_metadata(response_payload, self._base_url, provider_name, model_name),
			)
		...
		return OpenAICompatibleChatResult(
			...
			metadata=_chat_metadata(final_payload, self._base_url, provider_name, model_name),
		)
	...
	def _request_json(
		...
		try:
			...
			raise OpenAICompatibleRequestError(exc.code, _request_error_message(exc)) from exc
...
def _request_error_message(exc: HTTPError) -> str:
	...
	if isinstance(error_payload, str) and error_payload:
		return f"OpenAI-compatible request failed with status {exc.code}: {error_payload}"
	return f"OpenAI-compatible request failed with status {exc.code}"
