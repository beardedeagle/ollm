...
def resolve_audio_input(
    ...
    *,
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        return _resolved_audio_input(audio_bytes, audio_format)
    if parsed.scheme == "file":
        audio_bytes, audio_format = _read_local_audio(
            path=Path(unquote(parsed.path)), max_bytes=max_bytes
        )
        return _resolved_audio_input(audio_bytes, audio_format)
    ...
    if audio_path.exists() and audio_path.is_file():
        audio_bytes, audio_format = _read_local_audio(
            path=audio_path, max_bytes=max_bytes
        )
        return _resolved_audio_input(audio_bytes, audio_format)
...
def _decode_data_url_image(value: str, *, max_bytes: int) -> bytes:
    ...
    try:
        ...
        raise ValueError(
            "Data URL image inputs for Ollama contain invalid base64 data"
        ) from exc
...
def _decode_data_url_audio(value: str, *, max_bytes: int) -> tuple[bytes, str]:
    ...
    if audio_format is None:
        raise ValueError(
            "Data URL audio inputs currently support only WAV and MP3 media types"
        )
    if ";base64" not in header.lower():
...
def _read_local_audio(path: Path, *, max_bytes: int) -> tuple[bytes, str]:
    ...
    if audio_format is None:
        raise ValueError(
            "Audio inputs currently support only local .wav and .mp3 files"
        )
    return (
    ...
    try:
        with urlopen(request, timeout=timeout_seconds) as response:
            ...
            return _read_bounded_bytes(
                response, max_bytes=max_bytes, kind_label="image"
            )
    ...
    *,
    ...
    try:
        with urlopen(request, timeout=timeout_seconds) as response:
            ...
            audio_bytes = _read_bounded_bytes(
                response, max_bytes=max_bytes, kind_label="audio"
            )
...
def _read_bounded_bytes(
    response, *, max_bytes: int, kind_label: str = "media"
) -> bytes:
    buffer = bytearray()
    while True:
        ...
        if len(buffer) > max_bytes:
            raise ValueError(
                f"Remote {kind_label} URL exceeds the {max_bytes} byte download limit"
            )
    if not buffer:
