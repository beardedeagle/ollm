...
def build_prompt_command(
    ...
    *,
    ...
    if not use_specialization:
        command.append("--no-specialization")
...
def resolve_ollm_executable() -> Path:
    candidate = Path(sys.executable).resolve().with_name("ollm")
    if candidate.exists():
        return candidate
