...
MODEL_CATALOG: dict[str, ModelCatalogEntry] = {
    ...
    "qwen3-next-80B": ModelCatalogEntry(
        ...
        summary="Large MoE local chat model with custom disk-cache support.",
        repo_id="Qwen/Qwen3-Next-80B-A3B-Instruct",
        modalities=(ModelModality.TEXT,),
    ),
    "gemma3-12B": ModelCatalogEntry(
        model_id="gemma3-12B",
        summary="Gemma model supporting text and image chat.",
    ...
    "voxtral-small-24B": ModelCatalogEntry(
        ...
        summary="Voxtral model supporting text and audio chat.",
...
def get_model_catalog_entry(model_id: str) -> ModelCatalogEntry:
    if model_id not in MODEL_CATALOG:
        supported = ", ".join(sorted(MODEL_CATALOG))
        raise ValueError(f"Unsupported model id '{model_id}'. Supported ids: {supported}")
