...
class MyQwen3NextSparseMoeBlock(Qwen3NextSparseMoeBlock, loaderLayer):
    def forward_stacked(
    ...
    ) -> torch.Tensor:  # stacked+chunked experimental
        ...
        def stack_params(attr):
            return torch.stack(
                [getattr(self.experts[eid], attr).weight for eid in expert_ids], dim=0
            )
        ...
        # routing weights
        rw = (
            routing_weights[token_pos, local_rank].unsqueeze(-1).to(out.dtype)
        )  # out of index here
        out = out * rw
...
class MyQwen3NextForCausalLM(Qwen3NextForCausalLM):
    ...
    def offload_layers_to_gpu_cpu(self, gpu_layers_num=0, cpu_layers_num=0):
        ...
        layer_idx = 0
        while (
            gpu_layers_num > 0 or cpu_layers_num > 0
        ) and layer_idx < self.num_hidden_layers:
...
class Qwen3NextMultiTokenPredictor(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [Qwen3NextDecoderLayer(config, layer_idx) for layer_idx in range(1)]
        )
    ...
    def forward(self, **args):
        if self.logits2 is None:
            ...
            inputs_embeds = self.model.embed_tokens(
                args["input_ids"]
            )  # double computation, can be taken from main model
            outputs = self.mtp(
                hidden_states, inputs_embeds, position_ids, position_embeddings
            )
            ...
            self.input_ids, self.cache_position = (
                args["input_ids"],
                args["cache_position"],
            )
        ...
        return MoeCausalLMOutputWithPast(logits=logits, past_key_values=past_key_values)
