...
def online_chunked_grouped_attention_rope_no_mask(
    ...
    for hkv_idx, q_head_idxs in enumerate(groups):
        ...
        for q_start in range(0, Lq, q_block_size):
            ...
            m = torch.full(
                (B, Hq_g, Bq), float("-inf"), device=device, dtype=torch.float32
            )
            ...
            for k_start in range(0, Lk, k_block_size):
                ...
                # scores: (B, Hq_g, Bq, Bk)
                scores = (
                    torch.einsum("b h q d, b k d -> b h q k", q_block, k_block) * scale
                )

                # compute local max per row over k
                local_max = torch.amax(scores, dim=-1)  # (B, Hq_g, Bq)
                exp_scores = torch.exp(
                    scores - local_max.unsqueeze(-1)
                )  # (B, Hq_g, Bq, Bk)
                sum_exp = exp_scores.sum(dim=-1)  # (B, Hq_g, Bq)
                weighted_v_chunk = torch.einsum(
                    "b h q k, b k d -> b h q d", exp_scores, v_block
                )  # (B, Hq_g, Bq, D)
                ...
                if merge_idx.any():
                    ...
                    wv[merge_idx] = wv_old * alpha.unsqueeze(
                        -1
                    ) + wv_new * beta.unsqueeze(-1)
                    m[merge_idx] = new_m
...
def online_chunked_grouped_attention_rope(
    ...
    for hkv_idx, q_head_idxs in enumerate(groups):
        ...
        # q_sub: (B, Hq_g, Lq, D)
        q_sub = q[:, q_head_idxs].to(
            torch.float32
        )  # may copy but simpler; small Hq_g usually
        ...
        for q_start in range(0, Lq, q_block_size):
            ...
            for k_start in range(0, Lk, k_block_size):
                ...
                if causal:
                    ...
                    mask = k_pos_block.unsqueeze(1).unsqueeze(
                        2
                    ) > q_pos_block.unsqueeze(1).unsqueeze(3)
                else:
...
if __name__ == "__main__":
    ...
    out = online_chunked_grouped_attention_rope(
        q, k, v, q_block_size=64, k_block_size=128
    )
