...
def _coerce_hidden_layer_count(value: object) -> int:
	if not isinstance(value, int):
		raise RuntimeError("Expected transformer config to expose an integer hidden layer count")
	return value
...
class MyGemma3ForConditionalGeneration(Gemma3ForConditionalGeneration, oForGeneration):
	def __init__(self, config: Gemma3Config):
		super().__init__(config)
		self.num_hidden_layers = _coerce_hidden_layer_count(cast(Gemma3TextConfig, config.text_config).num_hidden_layers)
